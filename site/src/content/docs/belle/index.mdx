...
Not to be confused with a [stack overflow](#stack-overflows), a *value overflow* occurs when the CPU has a register that overflows its maximum capacity when an operation is performed on it. When a register overflows its value, the *overflow flag* is set, and the machine continues to operate as normal.
...
The debugger *can* crash if the `run` command is executed, however stepping through a program with `e` will never cause a crash.
